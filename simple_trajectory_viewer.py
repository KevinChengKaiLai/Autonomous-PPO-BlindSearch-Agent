...
def record_and_visualize_trajectory():
    """記錄並視覺化模型軌跡"""
...
def run_multiple_episodes_analysis(n_episodes=5):
    """運行多個episode並分析"""
    
    print(f"=== 多Episode軌跡分析 (運行 {n_episodes} 個episode) ===")
...
if __name__ == "__main__":
    ...
    if choice == "1":
        record_and_visualize_trajectory()
    elif choice == "2":
        n_episodes = input("請輸入episode數量 (預設5): ").strip()
        n_episodes = int(n_episodes) if n_episodes else 5
        run_multiple_episodes_analysis(n_episodes)
    else:
        print("無效選擇，運行單軌跡分析...")
        record_and_visualize_trajectory() 